...
import (
	...
	controllerconfig "github.com/ironcore-dev/gardener-extension-provider-metal/pkg/apis/config"
...
func (a *actuator) patchBackupBucketStatus(ctx context.Context, backupBucket *extensionsv1alpha1.BackupBucket, secretData map[string][]byte, endpoint string) error {
	...
	accessKeyID, ok := secretData[metal.BucketAccessKeyID]
	if !ok {
		return fmt.Errorf("missing %q field in secret", metal.BucketAccessKeyID)
	}

	secretAccessKey, ok := secretData[metal.BucketSecretAccessKey]
	if !ok {
		return fmt.Errorf("missing %q field in secret", metal.BucketSecretAccessKey)
	}

	accessSecretData := map[string][]byte{}
	accessSecretData[metal.AccessKeyID] = []byte(accessKeyID)
	accessSecretData[metal.SecretAccessKey] = []byte(secretAccessKey)
	accessSecretData[metal.Endpoint] = []byte(endpoint)
