...
func GetS3ClientFromS3ClientSecret(secret *corev1.Secret) (*s3.S3, error) {
	...
	accessKeyID, ok := secret.Data[metal.AccessKeyID]
	if !ok {
		return nil, fmt.Errorf("missing %q field in secret", metal.AccessKeyID)
	}

	secretAccessKey, ok := secret.Data[metal.SecretAccessKey]
	if !ok {
		return nil, fmt.Errorf("missing %q field in secret", metal.SecretAccessKey)
	}

	endpoint, ok := secret.Data[metal.Endpoint]
	if !ok {
		return nil, fmt.Errorf("missing %q field in secret", metal.Endpoint)
	}
